...
# 数据字段
每次执行时数据字段的含义或者定义不清楚的，需查看E:\file\qwen\basic\数据字典.md，文件中有对不同字段含义的解释，如果仍然有不理解的数据字段需标注出来

# 文件存储
- **创建新分析任务** 在 E:\file\qwen\data 中新建一个任务文件夹，文件夹名以本次任务分析的类型命名，代码保存在任务文件夹中创建一个文件夹code，分析的结果文件直接放在任务文件夹下
- **已有的分析任务** 直接将析的结果文件直接放在已有的同任务的任务文件夹下